        ...
        <tbody>
          ...
          {purchaseHistory?.map((element: PurchaseLog) => {
            return <tr key={element.item.quantity}>
              ...
              <td>{element.received?<button onClick={()=>navigate(`/review/new/${element.id}`)} className="bg-white rounded border-black border text-sm w-auto h-6 font-bold hover:text-teal-600"><IonIcon name="pencil-sharp"/>Write a review</button>:<button className="bg-white rounded border-black border text-sm w-auto h-6 font-bold hover:text-teal-600"><IonIcon name="pencil-sharp"/>Write a review</button>}</td>
            </tr>;
          })}
        </tbody>
      </table>
